...
SpringBoot2.0版本以上该类已经被WebServerFactoryCustomizer代替

```@Beanpublic WebServerFactoryCustomizer<ConfigurableWebServerFactory> webServerFactoryCustomizer() {    return new WebServerFactoryCustomizer<ConfigurableWebServerFactory>() {        @Override        public void customize(ConfigurableWebServerFactory factory) {            factory.setPort(8082);        }    };}
@Bean
...
​	2）、导入虚拟机文件centos7-atguigu.ova；（勾选初始化网卡）
...
博客地址:https://www.cnblogs.com/wangxiayun/p/10064421.html
...
2、拉取镜像
[root@localhost ~]# docker pull tomcat #默认tomcat携带jre
...
```
最新支持的驱动为:com.mysql.cj.jdbc.Driver，上述驱动启动会报错
```
...
```properties
schema-*.sql(建表语句)、data-*.sql(数据操作语句)
...
spring:
  datasource:
    username: root
    password: 123456
    url: jdbc:mysql://172.16.80.34:3307/jdbc?useUnicode=true&characterEncoding=UTF8
...
配置完后进入: http://localhost:8080/druid/index.html
